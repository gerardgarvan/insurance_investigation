...
write_combined_report <- function(df, output_path) {
  ...
  # Row 1: Title
  wb$add_data(
    sheet = "Summary", x = "Combined Unmatched Code Investigation",
  ...
  wb$add_data(
    sheet = "Summary", x = "By Classification:",
    start_row = 5, start_col = 1
  )
  wb$add_font(
    sheet = "Summary", dims = "A5",
  ...
  for (i in seq_along(ct_headers)) {
    wb$add_data(
      sheet = "Summary", x = ct_headers[i],
      start_row = ct_header_row, start_col = i
    )
  }
  wb$add_fill(
    sheet = "Summary", dims = glue("A{ct_header_row}:D{ct_header_row}"),
    color = wb_color("FF374151")
  )
  wb$add_font(
    sheet = "Summary", dims = glue("A{ct_header_row}:D{ct_header_row}"),
  ...
  for (i in seq_len(nrow(code_type_summary))) {
    ...
    wb$add_data(
      sheet = "Summary", x = code_type_summary$n_records[i],
      start_row = row_num, start_col = 3
    )
    wb$add_data(
      sheet = "Summary", x = code_type_summary$n_patients[i],
      start_row = row_num, start_col = 4
    )
    wb$add_numfmt(sheet = "Summary", dims = glue("B{row_num}:D{row_num}"), numfmt = "#,##0")
  }
  ...
  for (i in seq_along(st_headers)) {
    wb$add_data(
      sheet = "Summary", x = st_headers[i],
      start_row = st_header_row, start_col = i
    )
  }
  wb$add_fill(
    sheet = "Summary", dims = glue("A{st_header_row}:D{st_header_row}"),
    color = wb_color("FF374151")
  )
  wb$add_font(
    sheet = "Summary", dims = glue("A{st_header_row}:D{st_header_row}"),
  ...
  for (i in seq_len(nrow(source_summary))) {
    ...
    wb$add_data(
      sheet = "Summary", x = source_summary$n_records[i],
      start_row = row_num, start_col = 3
    )
    wb$add_data(
      sheet = "Summary", x = source_summary$n_patients[i],
      start_row = row_num, start_col = 4
    )
    wb$add_numfmt(sheet = "Summary", dims = glue("B{row_num}:D{row_num}"), numfmt = "#,##0")
  }
  ...
  for (category in category_order) {
    ...
    # Row 1: Title
    wb$add_data(
      sheet = sheet_name, x = "Combined Unmatched Code Investigation",
      start_row = 1, start_col = 1
    )
    wb$add_font(
    ...
    headers <- c(
      "Code", "Description", "Code Type", "Source Table",
