...
| `R/115_zip_stability_counts.R` | Read-only Phase 139 investigation (amended by 139-05-PATCH.md), source: team meeting notes 08/04 and 07/21-24/2026, informing (not deciding) the team's carry-forward/imputation design for time-varying ZIP data. Part A (address-history universe, `addr_coal` via local `coalesce_zip5()`): per-patient ZIP9/ZIP5 transition counts, +4-only transitions, exposure-denominator rate, gap-time distribution (median/IQR/deciles + histogram), and a record-anchored (not spell-anchored) carry-forward hold-out validation curve (A-06). Part B (cohort-restricted encounter universe, via `get_hl_patient_ids()`): per-encounter ZIP-availability classification and S1-S4 imputation-scenario occurrence counts, both an ordered mutually-exclusive assignment and unordered eligible-for counts with backward/forward direction split, at encounter and patient level. Part C: stepwise completeness waterfall (C-01) and a cohort-scoped C-02 reconciliation (`compute_c02()`) against a ~26-patient control total (+/-5 tolerance), with a pre-filter comparison and a loud console + xlsx (UF_ORANGE cell) flag if it fails to reconcile. Writes a UF-branded 8-sheet styled xlsx `output/zip_stability_counts_YYYYMMDD.xlsx` (KEY, A_stability_patient, A_stability_summary, A_validation_curve, B_scenario_counts, B_direction_split, C_completeness, QC). Dual probe gate (CSV then ENCOUNTER/DuckDB); exits gracefully if either is absent. | 139 |
| `R/116_encounter_ses_index.R` | Encounter-level SES index linkage. Pulls ENCOUNTER rows from DuckDB scoped to PATIDs present in `LDS_ADDRESS_HISTORY` (an address-coverage filter, not an HL-cohort filter — see D-07), resolves ZIP9/ZIP5 per encounter via `get_zip9_at_date() \| approximate_zip9()` (including Phase 144 Tier 3 centroid fallback), and joins four SES index scores from probe-first-gated reference files: SDI (Robert Graham Center, ZIP5), ADI (Neighborhood Atlas, ZIP9/block-group; pending P-03a), SVI (CDC, ZCTA≈ZIP5), RUCA (USDA 2020, ZIP5). Produces `output/encounter_ses_index_YYYYMMDD.rds` (one row per PATID × ENCOUNTERID × ADMIT_DATE with ZIP9, ZIP5, match_type, zip9_source, sdi_score, adi_natrank, svi_score, ruca_code, ruca_category) and `output/encounter_ses_index_summary_YYYYMMDD.xlsx` (3 sheets: ZIP source breakdown, index coverage, RUCA distribution). Registered in R/39 `investigation_scripts`. | 144 |
...
| `R/122_encounter_distance.R` | Encounter-ZIP to residence distance (Phase 152/153). Phase 153 (Plan 02): SECTION 4 replaced with build_patient_zip_calendar() + compute_encounter_distance() (zipcodeR, canonical); haversine retained as reference only. Output carries zip5_patient_source, days_offset, n_candidates_in_range, distance_status. E_completeness sheet adds a 5-step waterfall keyed on distance_status with row-for-row reconciliation stopifnot; QC adds n_candidates_in_range>1 count and nearest-fill days_offset distribution. | 00_config, utils/utils_address, utils/utils_zip_calendar |
| `R/147_surveillance_modality_frequency.R` | Surveillance modality frequency investigation (Phase 158 SURV-01..06 + Phase 159 LAB-01..LAB-07 + Phase 160 IMP-01..IMP-06). Pulls PROCEDURES and LAB_RESULT_CM from DuckDB via surv_code_where() pushdown with semi_join on a temp HL-ID table (Section 4A/4B). Section 4B: DISTINCT ID × code × date for lab analytes via analyte-rule matching (map_analyte_hits, build_analyte_events). Delegates all counting to utils_surveillance.R; analyte events merged into matched_all before classify_event_window. Writes two workbooks (INTERNAL unsuppressed + release suppressed) with sheets A_code_presence, A2_analyte_presence (INTERNAL only), A3_missing_analyte (INTERNAL=raw codes/names+unsuppressed counts; release=LOINC-level+suppressed), QC, near-miss block, and E_patient_modality_dates (INTERNAL only); and a patient-level .rds/.csv. Phase 160 adds: A3 missing-analyte diagnostic (summarise_missing_analyte, select_a3_sample, surv_sql_date_expr, rank_candidate_codes via tmp_surv_a3_days temp table), female-denominator eligible_sex columns for breast imaging modalities (modality_eligible_sex, compute_eligible_modality_stats, suppress_eligible_columns — complementary suppression: withholds eligible/other-sex pair when either side is 1–10), auto-generated Codeset_summary sheet (build_codeset_summary; replaces hand-built code_mapping_summary.xlsx), and L-5 eligibility invariant guard. Inputs: data/reference/surveillance_codeset.xlsx (sheets: Modalities, Codes, Lab_Analytes, Lab_Analytes_Excluded). Run constants: A3_SEED (2026L), A3_MAX_DAYS (5000L), A3_TOP_N (25L). Outputs: output/surveillance_modality_frequency_INTERNAL_\<date\>.xlsx, output/surveillance_modality_frequency_\<date\>.xlsx, output/surveillance_patient_modality_dates_\<date\>.rds, output/surveillance_patient_modality_dates_\<date\>.csv (INTERNAL; per-patient modality dates, stays on HiPerGator). Requirements: LAB-01..LAB-07, IMP-01..IMP-06. | 00_config, utils/utils_surveillance, utils/utils_duckdb |
...
| `utils/utils_zip_calendar.R` | Phase 153 patient ZIP calendar and best-ZIP selection (AM rules 2-3: DIST-02/DIST-03). Defines `build_patient_zip_calendar(addr, study_end)` (one row per ID × ZIP × period, sentinel/NA excluded, open periods closed at 2025-03-31), `pick_best_zip(enc, cal)` (D-04 two-zone ranking: in-range ZIP9>ZIP5 first, then nearest-in-time regardless of ZIP tier; D-05 tie-break: earlier period preferred), and `compute_encounter_distance(enc, cal)` (calls pick_best_zip(), then zipcodeR::zip_distance() on distinct ZIP5 pairs; returns distance_mi, distance_km, distance_status ∈ {computed, facility_zip_missing, patient_zip_missing, zip_not_in_db}, plus provenance columns zip5_patient_source, days_offset, n_candidates_in_range). Sourced explicitly by R/122 (not auto-sourced by 00_config.R). get_zip9_at_date() is kept in utils_address.R for Phase 139/141 consumers and is NOT called by R/122. | R/122_encounter_distance |
| `utils/utils_surveillance.R` | Phase 158/159 surveillance modality frequency helpers. Phase 158: `load_surveillance_codeset()` (reads surveillance_codeset.xlsx, validates codeset_row_id uniqueness, enforces D-18/D-20/D-21/D-22 structural rules), `surv_code_where()` (SQL WHERE clause builder for DuckDB pushdown), `normalize_surv_code()`, `surv_components()`, `get_hl_any_dx_ids()` (DuckDB semi_join on temp HL-ID table), `match_coded_events()`, `build_component_events()`, `build_code_presence()`, `compute_followup()`, `classify_event_window()`, `compute_modality_stats()`, `build_patient_modality()`, `suppress_small()`, `suppress_table()`. Phase 159: `load_lab_analytes()` (reads Lab_Analytes sheet; validates panel/rule_type; enforces KIDNEY analyte_min_same_day excludes CREATININE, D-22), `load_modality_lookup()` (reads Modalities sheet; validates every modality has a LOINC/CPT prefix, errors on missing prefix), `map_analyte_hits()` (joins raw lab pull to analyte rules on code), `build_analyte_events()` (applies panel rules: exact/analyte_all_same_day/analyte_min_same_day; D-08..D-10), `build_analyte_presence()` (one row per Lab_Analytes code: n_results, n_patients, first/last date, review_note), `build_patient_modality_dates()` (one row per denominator patient; n_dates_\<modality\> columns, zeros never blank; SC-6 nesting enforced). Auto-sourced by 00_config.R. | 00_config (auto-sourced), R/147_surveillance_modality_frequency |
