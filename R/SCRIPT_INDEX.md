...
| `R/147_surveillance_modality_frequency.R` | Surveillance modality frequency investigation (Phase 158 SURV-01..06 + Phase 159 LAB-01..LAB-07). Pulls PROCEDURES and LAB_RESULT_CM from DuckDB via surv_code_where() pushdown with semi_join on a temp HL-ID table (Section 4A/4B). Section 4B: DISTINCT ID × code × date for lab analytes via analyte-rule matching (map_analyte_hits, build_analyte_events). Delegates all counting to utils_surveillance.R; analyte events merged into matched_all before classify_event_window. Writes two workbooks (INTERNAL unsuppressed + release suppressed) with sheets A_code_presence, A2_analyte_presence (INTERNAL only), QC, near-miss block, and E_patient_modality_dates (INTERNAL only); and a patient-level .rds/.csv. Inputs: data/reference/surveillance_codeset.xlsx (sheets: Modalities, Codes, Lab_Analytes). Outputs: output/surveillance_modality_frequency_INTERNAL_\<date\>.xlsx, output/surveillance_modality_frequency_\<date\>.xlsx, output/surveillance_patient_modality_dates_\<date\>.rds, output/surveillance_patient_modality_dates_\<date\>.csv (INTERNAL; per-patient modality dates, stays on HiPerGator). Requirements: LAB-01..LAB-07. | 00_config, utils/utils_surveillance, utils/utils_duckdb |
