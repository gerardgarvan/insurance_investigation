...
| `R/147_surveillance_modality_frequency.R` | Surveillance modality frequency investigation (Phase 158; SURV-01..06). Pulls PROCEDURES and LAB_RESULT_CM from DuckDB via surv_code_where() pushdown with semi_join on a temp HL-ID table. Delegates all counting to utils_surveillance.R: match_coded_events, build_code_presence, compute_followup, classify_event_window, compute_modality_stats, build_patient_modality, suppress_table. Writes two workbooks (INTERNAL unsuppressed + release suppressed) and a patient-level .rds. Inputs: data/reference/surveillance_codeset.xlsx. Outputs: output/surveillance_modality_frequency_INTERNAL_\<date\>.xlsx, output/surveillance_modality_frequency_\<date\>.xlsx, output/surveillance_modality_patient_\<date\>.rds. | 00_config, utils/utils_surveillance, utils/utils_duckdb |
