...
## Current Milestone: v3.5 Encounter Distance (AM §4)

**Goal:** Implement the Analytic Manual §4 cleaning rules for patient-to-encounter distance exactly as written, resolve both Unresolved Issues (binary cutoff, histogram), and write results back into AM §4 so the domain is no longer blank or provisional.

**Target features:**
- Distance computation via `zipcodeR::zip_distance()` on ZIP5 pairs, with ZIP9→ZIP5 reduction upstream (DIST-01, DIST-02)
- Patient ZIP calendar + best-ZIP-per-encounter selection with nearest-in-time fill and provenance column (DIST-03)
- Histogram deliverable in UF colors (encounter- and patient-level, linear + log scale) + distribution summary xlsx (DIST-04)
- Binary `far_from_care` indicator behind `CONFIG$distance_cutoff_mi` + data-driven cutoff memo for Amy/Erin (DIST-05)
- AM §4 write-back (Expected Structure, Observed Issues, Cleaning Rules) + AM §3 decision log rows D-01..D-06 (DIST-06)
- Script registration in `R/39`, `R/88` smoke test, `R/SCRIPT_INDEX.md`, `tests/testthat/` (DIST-07)

**Open before Phase 153 code:** D-02 (ZIP at encounter date vs. one ZIP per patient), D-04 (rule 3 covers patient ZIP only, not missing facility ZIP), and two 153-00 design choices (calendar from address history only; ZIP9 outranks a closer-fitting ZIP5) — resolved in `153-CONTEXT` before 153-01. D-03 already resolved (2026-09-17): nearest to encounter date, either direction, tie → earlier address. D-06 (cutoff value) deferred to Amy/Erin via Phase 155 memo.
