...
check(
  glue("Treatment decade complete: 10/10 scripts (found {treatment_found})"),
  treatment_found == 10
)
...
check(
  glue("Cancer decade scripts: {cancer_found}/14 present"),
  cancer_found >= 10
) # Allow for some Phase 66+ additions
...
check(
  glue("Payer/QA decade scripts: {payer_found}/10 present"),
  payer_found >= 8
)
