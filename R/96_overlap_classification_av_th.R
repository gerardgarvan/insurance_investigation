...
# Read Phase 33 Plan 01 same-date detail (AV+TH filtered)
...
message(glue("Loaded multi_source_same_date_detail_av_th.csv: {format(nrow(same_date_detail), big.mark=',')} rows"))
...
same_week_detail <- read_csv(
  file.path(output_dir, "multi_source_same_week_detail_av_th.csv"),
  col_types = cols(
    ID = col_character(),
...
message(glue("Loaded multi_source_same_week_detail_av_th.csv: {format(nrow(same_week_detail), big.mark=',')} rows"))
...
# Phase 33: Filter to AV+TH encounters only for field comparison
...
# --- CSV 1: classified_same_date_detail_av_th.csv ---
csv1 <- sd_pairs %>%
  select(
...
write_csv(csv1, file.path(output_dir, "classified_same_date_detail_av_th.csv"))
message(glue("  Written: classified_same_date_detail_av_th.csv ({format(nrow(csv1), big.mark=',')} rows)"))

# --- CSV 2: classified_same_week_detail_av_th.csv ---
csv2 <- sw_pairs %>%
  select(
...
write_csv(csv2, file.path(output_dir, "classified_same_week_detail_av_th.csv"))
message(glue("  Written: classified_same_week_detail_av_th.csv ({format(nrow(csv2), big.mark=',')} rows)"))

# --- CSV 3: per_site_overlap_profile_av_th.csv ---
...
message(glue("\n{strrep('=', 70)}"))
message("OVERLAP CLASSIFICATION AND RECOMMENDATIONS (AV+TH ONLY) -- SUMMARY")
message(glue("Phase 33: Overlap classification for AV+TH multi-source encounters"))
message(glue("{strrep('=', 70)}"))
...
message("  - classified_same_date_detail_av_th.csv")
message("  - classified_same_week_detail_av_th.csv")
message("  - per_site_overlap_profile_av_th.csv")
message("  - overlap_source_payer_completeness_av_th.csv")

message(glue("\nPhase 33 complete (AV+TH): {format(nrow(sd_pairs), big.mark=',')} same-date pairs and {format(nrow(sw_pairs), big.mark=',')} same-week pairs classified"))

message(glue("\n{strrep('=', 70)}"))
message("END OF OVERLAP CLASSIFICATION AND RECOMMENDATIONS (AV+TH ONLY)")
message(glue("{strrep('=', 70)}"))
