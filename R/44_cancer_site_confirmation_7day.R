...
OUTPUT_PATH <- build_output_path("tables", "cancer_site_confirmation_7day.xlsx")
...
# Filter to neoplasm codes — map-based helper excludes D5x-D9x (PATTERN-C fix)
dx_cancer <- dx_icd10 %>%
...
grand_total_patients_44 <- n_distinct(dx_cancer$ID)

totals_exact <- tibble(
  DX_norm = "TOTAL",
  category = "",
  total_patients = grand_total_patients_44,
...
# Row 1: Title
wb$add_data(
  sheet = SHEET1, x = "Cancer Site Confirmation - Exact Code (7-Day Gap)",
...
# Row 1: Title
wb$add_data(
  sheet = SHEET2, x = "Cancer Site Confirmation - Category Level (7-Day Gap)",
  start_row = 1, start_col = 1
)
wb$add_font(
