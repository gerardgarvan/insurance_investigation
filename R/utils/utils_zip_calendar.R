...
#   pick_best_zip() arrange() revised per D-04 (two-zone ranking) and D-05
...
#
#   compute_encounter_distance() -- tibble: all pick_best_zip() columns plus
...
#     is_sentinel_zip5() from R/utils/utils_address.R (auto-sourced by
#     R/00_config.R; do NOT redefine here)
...
# of negative (later period) on a tie — implements D-05.
#
pick_best_zip <- function(enc, cal) {
  enc |>
    ...
    # D-04 two-zone ranking (departure from Appendix A's single flat arrange):
    ...
    # desc(days_offset)         -- D-05 tie-break: positive offset (earlier period) first
...
#'   ZIP is reported as distance_status = "facility_zip_missing" (D-02).
#' @param cal Output of build_patient_zip_calendar().
#'
#' @return Tibble: all columns from pick_best_zip() (ID, ENCOUNTERID,
#'   ADMIT_DATE, zip9_patient, zip5_patient, zip5_patient_source, days_offset,
...
#'     data quality problem, not addressed by LDS_ADDRESS_HISTORY (D-02).
