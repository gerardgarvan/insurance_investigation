# ==============================================================================
# 26_generate_speedup_report.R -- DuckDB vs RDS speedup report generator
# ==============================================================================
#
# Phase 32: DBDIAG-03
...
#
# Usage:
#   source("R/26_generate_speedup_report.R")
#
# ==============================================================================
...
if (!file.exists(benchmark_path)) {
  stop(glue("Benchmark CSV not found: {benchmark_path}\n",
...
for (i in seq_len(nrow(speedup_table))) {
  ...
  report_lines <- c(report_lines,
    glue("| {row$script} | {round(row$rds_median, 2)} | {round(row$ddb_median, 2)} | ",
...
if (has_memory) {
  ...
  for (i in seq_len(nrow(speedup_table))) {
    row <- speedup_table[i, ]
    report_lines <- c(report_lines,
      glue("| {row$script} | {round(row$rds_memory_mb, 1)} | {round(row$ddb_memory_mb, 1)} | ",
...
for (i in seq_len(nrow(variance_rows))) {
  ...
  report_lines <- c(report_lines,
    glue("| {row$script} | {row$backend} | {round(row$median_sec, 2)} | ",
...
report_lines <- c(report_lines,
  ...
  "- Scripts dominated by CSV writing or HIPAA suppression overhead may show lower speedup,",
  ...
  "",
  "---",
  glue("*Report generated by R/26_generate_speedup_report.R on {Sys.Date()}*")
)
