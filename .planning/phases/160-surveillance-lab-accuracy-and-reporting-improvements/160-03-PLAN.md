...
must_haves:
  truths:
    - "A3 block 1 reconciles with the near-miss table (stopifnot); block 2 comes from a sampled ID x date join inside the database with Lab_Analytes LOINCs excluded in SQL"
    - "B/C modality rows use compute_eligible_modality_stats(); a stopifnot proves the all-patient B columns equal plain compute_modality_stats()"
    - "Release: suppress_eligible_columns() before standard suppression; A3 counts suppressed; A3 block 2 drops RAW-only candidates and raw code/name columns"
    - "Sheet order: KEY, A_code_presence, A2, A3_missing_analyte, B, C, D, E (INTERNAL only), QC, Codeset_summary"
    - "KEY carries the D-04 wording; QC reports sex distribution, A3 sample size, candidate rows, date-join coverage and the RAW-only flag"
---

<objective>
Wire the Phase 160 functions into R/147.
</objective>
...
@.planning/phases/160-surveillance-lab-accuracy-and-reporting-improvements/160-02-SUMMARY.md
@R/147_surveillance_modality_frequency.R
</context>
...
<task type="auto">
  ...
  <read_first>R/147_surveillance_modality_frequency.R — current file; R/utils/utils_duckdb.R — confirm the connection class, so `inherits(con, "duckdb_connection")` selects the DuckDB date expression</read_first>
  <files>R/147_surveillance_modality_frequency.R</files>
  <action>
Compare the current file with the Phase 159 version. The only change expected since then is the executor's wording in any loader message. If R/147 has other changes, stop and report them rather than overwrite. Otherwise replace it with the content below. It is the Phase 159 file with:
- A3 and path constants
- `elig` from the Modalities sheet
- DEMOGRAPHIC pull (`sex_lookup`)
- eligible-sex stats for modality rows, with an L-5 guard
- the A3 section: block 1, reconciliation stop, sample, database-side date join, ranking, RAW-only flag
- Codeset_summary
- new QC and KEY rows
- A3 and Codeset_summary sheets with their extra blocks
...
```
  ...
  <verify>
    <automated>Rscript -e "f <- 'R/147_surveillance_modality_frequency.R'; invisible(parse(f)); src <- paste(readLines(f), collapse = '\n'); for (k in c('summarise_missing_analyte', 'select_a3_sample', 'surv_sql_date_expr', 'tmp_surv_a3_days', 'rank_candidate_codes', 'compute_eligible_modality_stats', 'suppress_eligible_columns', 'build_codeset_summary', 'A3_missing_analyte', 'Codeset_summary', 'screening-population uptake', 'L-5: eligibility')) stopifnot(grepl(k, src, fixed = TRUE)); message('OK')"</automated>
  </verify>
  <done>R/147 parses and contains the Phase 160 wiring.</done>
</task>

</tasks>

<verification>
Parse and keyword checks above; the real run is 160-04. This script was run end-to-end against a SQLite fixture with the delivered codeset and crosswalk:
- every stop passed
- both workbooks had the specified sheet order
- block 2 ranked the substitute CO2 code first (lift 1) and hemoglobin at lift 0
- the male/unknown mammogram showed in the other-sex columns
- release suppression withheld the female/other-sex pair

DuckDB's own date expression could not be exercised there; the 50% coverage warning covers that at the checkpoint.
If `Rscript` is unavailable (Windows dev host), do not skip silently: run the structural fallback below, record in the SUMMARY that the testthat run is deferred, and 160-04 Task 2 runs it on HiPerGator first.
</verification>
